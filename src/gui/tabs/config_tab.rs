...
impl ConfigTab {
    pub fn new(state: Arc<Mutex<AppState>>, runtime: Arc<Runtime>) -> Self {
        ...
        for key_path in &config.pgp.team_keys {
            if let Ok(key_data) = std::fs::read(key_path) {
                ...
                if let Ok(key_infos) =
                    rust_r2::crypto::PgpHandler::get_all_keys_from_bytes(&key_data)
                {
        ...
        for file in &files_to_process {
            if let Some(path) = &file.path {
                ...
                if path_str.ends_with(".key")
                    || path_str.ends_with(".asc")
                    || path_str.ends_with(".gpg")
                    || path_str.ends_with(".pgp")
                {
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.horizontal(|ui| {
                            if self.show_secret {
                                ...
                                ui.add_enabled(
                                    false,
                                    egui::TextEdit::singleline(&mut masked.clone()),
                                );
                            }
                            if ui
                                .button(if self.show_secret {
                                    "👁"
                                } else {
                                    "👁‍🗨"
                                })
    ...
    fn save_config(&mut self) {
        ...
        state.config.pgp.team_keys = self
            .team_keys
            .iter()
            .map(|(path, _)| path.clone())
            .collect();
        state.config.pgp.secret_key_path = if self.secret_key_path.is_empty() {
            None
        } else {
            Some(self.secret_key_path.clone())
        };
        state.config.pgp.passphrase = if self.passphrase.is_empty() {
            None
        } else {
    ...
    fn test_connection(&mut self, ctx: &egui::Context) {
        ...
        // Update config before testing
        {
            ...
            app_state.config.pgp.team_keys = self
                .team_keys
                .iter()
                .map(|(path, _)| path.clone())
                .collect();
            app_state.config.pgp.secret_key_path = if self.secret_key_path.is_empty() {
                None
            } else {
                Some(self.secret_key_path.clone())
            };
            app_state.config.pgp.passphrase = if self.passphrase.is_empty() {
                None
            } else {
                Some(self.passphrase.clone())
            };
                        ...
                        Ok(_) => {
                            ...
                            for key_path in &config.pgp.team_keys {
                                if let Ok(key_data) = std::fs::read(key_path) {
                                    ...
                                    let _ = pgp_handler
                                        .load_keyring(&key_data, config.pgp.passphrase.as_deref());
                                }
                            }
                            ...
                            if !pgp_handler.has_secret_key() {
                                if let Some(secret_path) = &config.pgp.secret_key_path {
                                    if let Ok(key_data) = std::fs::read(secret_path) {
                                        let _ = pgp_handler.load_secret_key(
                                            &key_data,
                                            config.pgp.passphrase.as_deref(),
                                        );
                                    }
                                }
                            }
