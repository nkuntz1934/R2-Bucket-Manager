...
impl R2Client {
    ...
    pub async fn download_object(&self, key: &str) -> Result<Bytes> {
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow!("R2 download failed with status {}: {}", status, error_text));
        }
    ...
    pub async fn upload_object(&self, key: &str, data: Bytes) -> Result<()> {
        ...
        if !response.status().is_success() {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(anyhow!("R2 upload failed with status {}: {}", status, error_text));
        }
    ...
    pub async fn list_objects(&self, prefix: Option<&str>) -> Result<Vec<String>> {
        ...
        if !response.status().is_success() {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(anyhow!("R2 list failed with status {}: {}", status, error_text));
        }
    ...
    pub async fn delete_object(&self, key: &str) -> Result<()> {
        ...
        if !response.status().is_success() && response.status().as_u16() != 404 {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(anyhow!("R2 delete failed with status {}: {}", status, error_text));
        }
